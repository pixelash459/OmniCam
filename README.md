...
3. **PC** — easiest: download `OmniCam-PC-*-Setup.exe` from the [Releases page](https://github.com/pixelash459/OmniCam/releases) and run the installer (install [OBS Studio](https://obsproject.com) once for the virtual camera). From source instead: `cd pc`, create a venv, `pip install -r requirements.txt`, then `run.bat`.
